...
LDFLAGS += -ldl ${LVTKLIB}
CXXFLAGS += -Wl,--no-as-needed -Wno-narrowing -shared -fPIC -DPIC -Isrc/ -std=c++11 -lpd -Ilvtk/ -Ilibpd/pure-data/src/ -Ilibpd/libpd_wrapper/ -DPDINSTANCE
...
install_libpd: $(LIBPD_SO)
	cd libpd/ && make install $(LIBPD_FLAGS)
